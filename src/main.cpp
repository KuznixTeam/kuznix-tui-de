...
void animate_highlight(WINDOW* win, const std::vector<std::string>& items, int prev, int curr, int top, int visRows, int width) {
    ...
    int steps = std::min(std::abs(curr-prev), visRows);
    for (int i=1; i<=steps; ++i) {
        int temp = prev + i*dir;
        int idx = temp - top;
        if (idx >= 0 && idx < visRows) {
            wattron(win, COLOR_PAIR(CP_HIGHLIGHT));
            mvwprintw(win, idx, 2, "%-*s", width-4, items[temp].c_str());
...
void filter_list(const std::vector<std::string>& src, std::vector<std::string>& out, const std::string& pat) {
    out.clear();
    for (auto& bin : src) {
        if (pat.empty() || bin.find(pat) != std::string::npos)
            out.push_back(bin);
    }
}
...
void show_filter_dialog(int term_h, int term_w, std::string& filter, bool& apply) {
    ...
    while ((ch = wgetch(win))) {
        ...
        if (ch == KEY_BACKSPACE || ch == 127 || ch == 8) {
            if (!filter.empty()) {
                filter.pop_back();
                mvwprintw(win, 1, 16, "%-18s", filter.c_str());
                wmove(win, 1, 16+filter.size());
            }
        } else if (isprint(ch) && filter.size() < 16) {
            filter.push_back(ch);
            mvwaddch(win, 1, 16+filter.size()-1, ch);
            wmove(win, 1, 16+filter.size());
        }
        wrefresh(win);
    }
...
int main() {
    ...
    while (running) {
        ...
        getmaxyx(stdscr, term_h, term_w);
        wresize(header, 1, term_w);
        wresize(footer, 1, term_w);
        wresize(listwin, term_h-2, term_w);
        ...
        if (ch == 'q') {
            ...
            curs_set(0);
            getmaxyx(stdscr, term_h, term_w);
            header = newwin(1, term_w, 0, 0);
            footer = newwin(1, term_w, term_h-1, 0);
            listwin = newwin(term_h-2, term_w, 1, 0);
        } else if (ch == KEY_F(1)) {
            show_about_dialog(term_h, term_w);
        } else if (ch == 6) { // Ctrl+F
            ...
            if (doApply) {
                filter = newfilter;
                filter_list(all_binaries, filtered, filter);
                highlight = 0; top = 0;
            }
        }
    }
